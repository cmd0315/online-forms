...
class ApproveReceiveRequestCommandHandler implements CommandHandler {
	...
	* Handles the command.
	*
	* @param ApproveReceiveRequest $command
	*/
	public function handle($command) {
		...
		$onlineForm->stage = 2; //update stage
		...
		if($command->decisionOptions == 0) {
			...
			$onlineForm->received_by = $command->receiver;
		}
		else {
